...
var FileItemMenu = class {
    ...
    _doOpenWithVSCode() {
        let selectedFiles = this._desktopManager.getCurrentSelection(false);
        if (!selectedFiles || selectedFiles.length === 0) {
            selectedFiles = [this._currentFileItem];
        }
        if (!selectedFiles || selectedFiles.length === 0) {
            return;
        }

        const commandCandidates = ['code', 'code-insiders', 'codium'];
        const command = commandCandidates.find(program => GLib.find_program_in_path(program));
        if (!command) {
            let WindowError = new ShowErrorPopup.ShowErrorPopup(
                _('Visual Studio Code not found'),
                _('The "code" command is not available. Please install VS Code and ensure it is in your PATH.'),
                false
            );
            WindowError.run();
            return;
        ...
        for (let fileItem of selectedFiles) {
            if (fileItem && !fileItem.isSpecial) {
                params.push(fileItem.file.get_path());
            }
        }
        if (params.length === 1) {
            return;
        }
        DesktopIconsUtil.trySpawn(null, params);
    }
